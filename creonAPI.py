...
class CpStockChart:
    def __init__(self, queue, ui_num):
        self.windowQ = queue
        self.ui_num = ui_num
    ...
    def RequestDWM(self, code, dwm, count, caller: 'MainWindow', from_date=0, ohlcv_only=True):
        ...
        while count > rcv_count:
            ...
            if len(rcv_data['date']) == 0:  # 데이터가 없는 경우
                massage = f'[{dwm}] {code} 데이터 없음' 
                self.windowQ.put([self.ui_num['S로그텍스트'], massage])
                print(massage)
                return False
    ...
    def RequestMT(self, code, dwm, tick_range, count, caller: 'MainWindow', from_date=0, ohlcv_only=True):
        ...
        while count > rcv_count:
            ...
            if len(rcv_data['date']) == 0:  # 데이터가 없는 경우
                if dwm == 84: tick_unit = 'tick'
                elif dwm == 109: tick_unit = '1min'
                else: tick_unit = '5min'

                massage = f'[{tick_unit}] {code} 데이터 없음' 
                self.windowQ.put([self.ui_num['S로그텍스트'], massage])
                print(massage)
                return False
                # print(code, '데이터 없음')
