...
class CreonDatareaderCLI:
    def __init__(self, windowQ, uni_num):
        self.windowQ = windowQ
        self.objStockChart = creonAPI.CpStockChart(windowQ, uni_num)
    ...
    def update_price_db(self, db_path, tick_unit='day', ohlcv_only=False):
        ...
        if db_latest_list:
            ...
            if date0[0] > 99999999:
                if date1[0] - date0[0] == 5: # 5분 간격인 경우
                    tick_unit = '5min'
                elif date1[0] - date0[0] == 1: # 1분 간격인 경우
                    tick_unit = '1min'
                else:
                    tick_unit = 'tick'
        ...
        with sqlite3.connect(db_path) as self.con:
            cursor = self.con.cursor()
            fetch_code_df_len = len(fetch_code_df)
            tqdm_range = tqdm.trange(fetch_code_df_len, ncols=100)
            # tqdm_range = tqdm.trange(len(fetch_code_df[:1]), ncols=100)
            for i in tqdm_range:
                code = fetch_code_df.iloc[i]
                update_status_msg = '[{}] {}'.format(code[0], code[1])
                update_msg = preformat_cjk(update_status_msg, 20)
                tqdm_range.set_description(update_msg)

                massage = f'[{tick_unit}] {i}/{fetch_code_df_len} {update_msg}' 
                ...
                # print(code[0], code[1], from_date, tick_unit)

                if tick_unit == 'day':  # 일봉 데이터 받기
                    if self.objStockChart.RequestDWM(code[0], ord('D'), count, self, from_date, ohlcv_only) == False:
                        continue
                elif tick_unit == 'tick':  # 틱 데이터 받기
                    if self.objStockChart.RequestMT(code[0], ord('T'), tick_range, count, self, from_date, ohlcv_only) == False:
                        continue
                    ...
                    if self.objStockChart.RequestDWM(code[0], ord('M'), count, self, from_date, ohlcv_only) == False:
                        continue
                self.df = pd.DataFrame(self.rcv_data, columns=columns, index=self.rcv_data['date'])
                # print(df, len(df))
                ...
                self.df = self.df.iloc[::-1]
                self.df.to_sql(code[0], self.con, if_exists='append', index_label='date')
...
#     # parser.add_argument('--db_file_path', required=True, type=str, default='./db/stock_price(tick).db')
#     # parser.add_argument('--tick_unit', required=False, type=str, default='tick', help='{tick, 1min, 5min, day, week, month}')
#     # parser.add_argument('--ohlcv_only', required=False, type=int, default=0, help='0: False, 1: True')
#     # args = parser.parse_args()
#     db_file_path = './db/stock_price(tick).db'
#     tick_unit = 'tick'

#     creon = CreonDatareaderCLI()
#     # creon.update_price_db(args.db_file_path, args.tick_unit, args.ohlcv_only==1)
#     creon.update_price_db(db_file_path, tick_unit, False)
